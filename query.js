...
*/
  ...
  // 3.5 Write to file instead
  const stamp = Date.now();
  fs.writeFileSync(`valiData_${stamp}.txt`, '');
  eraStakers.map(async ([key, data]) => {
    fs.appendFileSync(`valiData_${stamp}.txt`, `Stash Account: ${key.args[1]}\n`);
    fs.appendFileSync(`valiData_${stamp}.txt`, `Total: ${data.total}\n`);
    fs.appendFileSync(`valiData_${stamp}.txt`, `Own: ${data.own}\n`);
    fs.appendFileSync(`valiData_${stamp}.txt`, '-----------------\n\n');
  });
