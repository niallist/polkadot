...
const main = async () => {
  ...
  //4.  What are events and tx fee [Objects]
  try {
    const result = await api.post('/transactions_search', {
      network: 'polkadot',
      chain_id: 'mainnet',
    ...
    console.log('first result\'s tx fee is', result.data[0]['transaction_fee']);
    console.log('first result\'s events are', result.data[0]['events']);
    console.log('first result\'s sub events are', result.data[0]['events']['sub'][0]);
  } catch (e) {
    console.log('Error getting extra credit,', e.message);
  }
};
